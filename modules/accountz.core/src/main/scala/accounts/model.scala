...
object Account {
  ...
  private def validateOpenCloseDate(
      od: Date,
      cd: Option[Date]
  ): ValidationResult[(Option[Date], Option[Date])] =
    cd.map { c =>
      if (c before od)
  ...
  private def validateCloseDate(
    ...
    if (cd before a.dateOfOpen.get)
      s"Close date [$cd] cannot be earlier than open date [${a.dateOfOpen.get}]".invalidNec
    else cd.validNec
  }
  ...
  private def checkBalance(
    ...
    if (amount < 0 && a.balance.amount < -amount)
      s"Insufficient amount in ${a.no} to debit".invalidNec
    else a.validNec
}
